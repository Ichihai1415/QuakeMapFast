...
namespace QuakeMapFast
{
    internal partial class DataPro
    {
        ...
        public static void DetailScale(P2PQuake_JMAQuake? json)
        {
            ...
            var size = Math.Min(108f, Math.Max(10.8f, zoom / 7.5f));//size=>(範囲(単位:度))
            if (debug)
            ...
            // ImageAttributesを使用してColorMatrixを設定
            ...
            for (var i = 0; i < 10; i++)
            {
                ...
                if (pointDict.TryGetValue(scale, out var values))
                {
                    ...
                    foreach (var (lat, lon) in values)
                    {
                        ...
                        g.DrawImage(icon, new Rectangle((int)x, (int)y, (int)size, (int)size), 0, 0, icon.Width, icon.Height, GraphicsUnit.Pixel, imageAttributes);
